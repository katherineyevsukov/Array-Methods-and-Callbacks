...
function getFinals(array){
    const finalTeams = array.filter(function(games){
        if(games.Stage === 'Final'){
            return true;
        }
    }); return finalTeams;
}
...
// function getFinals(array){
//     const finalTeams = array.filter(games => (games.Stage === 'Final')
//     ); return finalTeams
// }
...
function getYears(array, getFinalsCB) {
    const finalsGames = getFinalsCB(array);
    const years = finalsGames.map(function(games){
        return games.Year
    }); return years;
}
...
// function getYears(array, getFinalsCB){
//     const finalsGames = getFinalsCB(array);
//     const years = finalsGames.map(games => games.Year);
//     return years;
// }
...
function getWinners(array, getFinalsCB) {
        let winners = [];
        getFinalsCB(array).filter(function(games){
        if(games['Home Team Goals'] > games['Away Team Goals']){
            winners.push(games['Home Team Name']);
        }else {
            winners.push(games['Away Team Name']);
        }
    }); return winners
}
   // arrow function
...
function getWinnersByYear(array, getYearsCB, getWinnersCB) {
    ...
    let winners = getWinnersCB(array);
    let final = years.map(function(year, index) {
...
function getAverageGoals(getFinalsCB) {
   const averageGoals = getFinalsCB.reduce(function(acc, cv){
       return acc + cv['Home Team Goals'] + cv['Away Team Goals']}, 0);
